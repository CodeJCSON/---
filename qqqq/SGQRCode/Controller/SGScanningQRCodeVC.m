...
- (void)createNav{
    ...
    UIImageView *upImageV = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 64)];
    upImageV.image = [UIImage imageNamed:@"Navi"];
    [self.view addSubview:upImageV];
    
    upImageV.userInteractionEnabled = YES;
    UIButton *leftBtn = [[UIButton alloc] initWithFrame:CGRectMake(15, 28, 30, 30)];
    [leftBtn setImage:[UIImage imageNamed:@"action_bar_back"] forState:UIControlStateNormal];
    [leftBtn setImageEdgeInsets:UIEdgeInsetsMake(5, 5, 5, 5)];
    [leftBtn addTarget:self action:@selector(btnDo:) forControlEvents:UIControlEventTouchUpInside];
    [upImageV addSubview:leftBtn];
    
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(([UIScreen mainScreen].bounds.size.width - 100)/2, (64 - 30)/2+8, 100, 30)];
    label.text = @"扫一扫";
    label.textColor = [UIColor whiteColor];
    label.textAlignment = NSTextAlignmentCenter;
    [upImageV addSubview:label];
}
- (void)btnDo:(UIButton *)btn
{

    [self dismissViewControllerAnimated:YES completion:nil];
}
-(void)setResulestBlock:(void (^)(NSString *))ResulestBlock
{

    _ResulestBlock = ResulestBlock;
...
//            [self.navigationController pushViewController:jumpVC animated:YES];
            [self dismissViewControllerAnimated:YES completion:^{
                //                if ([self.delegate respondsToSelector:@selector(returnSymbol:)]) {
                //                    [self.delegate returnSymbol:obj.stringValue];
                //                }
